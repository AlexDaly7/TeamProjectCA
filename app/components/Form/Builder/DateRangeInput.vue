...
const { value, setValue, handleBlur } = useField<{
    start?: Date;
    end?: Date;
}>(props.name);
...
<template>
    <DateRangePickerRoot
        ...
        <DateRangePickerField
            v-slot="{ segments }"
            class="h-8 pl-4 flex select-none items-center rounded-md text-center p-1 bg-slate-700 ring-md data-invalid:border-red-500"
            :class="{
            ...
            <DateRangePickerTrigger
                ...
                <Icon name="hugeicons:calendar-02" size="20" />
            </DateRangePickerTrigger>
        </DateRangePickerField>

        <DateRangePickerContent :side-offset="4" class="rounded-xl bg-main-700 border border-main-500 shadow-sm z-150">
                            ...
                            <DateRangePickerGridRow
                                ...
                                <DateRangePickerCell
                                    ...
                                    <DateRangePickerCellTrigger
                                        :day="weekDate"
                                        :month="month.value"
                                        class="relative flex items-center justify-center rounded-full whitespace-nowrap text-sm font-normal size-8 outline-none focus:ring-2 focus:ring-black data-outside-view:text-txt-primary/30 data-selected:bg-sky-500! data-selected:text-main-900 hover:bg-sky-950 data-highlighted:bg-sky-400/10 data-unavailable:pointer-events-none data-unavailable:text-txt-primary/40 data-unavailable:line-through before:absolute before:top-1.25 before:hidden before:rounded-full before:size-1 before:bg-main-500 data-today:before:block data-today:before:bg-sky-500" />
                                </DateRangePickerCell>
                            </DateRangePickerGridRow>
                        </DateRangePickerGridBody>
                    </DateRangePickerGrid>
